...
import { Loader2, Lock, Unlock, MessageSquare, Clock, ChevronUp, ChevronDown, CheckCircle2, FlaskConical, Plus } from 'lucide-react';
...
export default function GamePage() {
    ...
    const handleCombineClues = () => {
        ...
        const combination = CASE_1.evidence_combinations.find(
            ec => (ec.clue_id_1 === c1.id && ec.clue_id_2 === c2.id) ||
    ...
    useEffect(() => {
        // Initialize and shuffle timeline events
        const events = [...CASE_1.timeline_events];
        // Simple shuffle
    ...
    useEffect(() => {
        ...
        if (storedRole) {
            const initialMyClues = CASE_1.clues.filter(
                c => c.visible_to === storedRole || c.visible_to === 'BOTH'
            );
            setMyClues(initialMyClues);
        }
        setLoading(false);
    ...
    return (
        <main className="min-h-screen bg-neutral-950 text-neutral-100 p-4 md:p-8">
            <header className="flex justify-between items-center mb-8 border-b border-neutral-800 pb-4">
                <div>
                    <h1 className="text-2xl font-bold text-red-600">{CASE_1.title}</h1>
                    <p className="text-sm text-neutral-400">Oda: {code}</p>
                </div>
                <div className="text-right">
                    <p className="font-bold">{role === 'DETECTIVE_A' ? 'Baş Dedektif' : 'Yardımcı Dedektif'}</p>
                    <p className="text-xs text-neutral-500">Rolün</p>
                </div>
            </header>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Left Column: My Clues */}
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Suspects */}
                        <div className="bg-neutral-900/50 p-6 rounded-2xl border border-neutral-800 col-span-full">
                            <h3 className="text-lg font-bold mb-4 text-neutral-300">Şüpheliler</h3>
                            <div className="grid grid-cols-2 gap-4">
                                {CASE_1.suspects.map(suspect => (
                                    <div
                                        key={suspect.id}
                                        onClick={() => handleSuspectClick(suspect)}
                                        className="bg-neutral-950 p-3 rounded-lg border border-neutral-800 hover:border-red-900/50 transition-colors cursor-pointer"
                                    >
                                        <p className="font-bold">{suspect.name}</p>
                                        <p className="text-xs text-neutral-500">{suspect.bio}</p>
                                        <p className="text-xs text-red-400 mt-2 font-semibold">Sorgulamak için tıkla</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 max-w-lg w-full space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {CASE_1.suspects.map(suspect => {
                                const isMySelection = selectedSuspect === suspect.id;
                                const partnerRole = role === 'DETECTIVE_A' ? 'DETECTIVE_B' : 'DETECTIVE_A';
                                const partnerVoteId = myVotes?.[partnerRole];
                                const isPartnerVote = partnerVoteId === suspect.id;

                                return (
                                    <button
                                        key={suspect.id}
                                        onClick={() => setSelectedSuspect(suspect.id)}
                                        className={`p-4 rounded-xl border text-left transition-all relative overflow-hidden ${isMySelection
                                            ? 'bg-red-900/30 border-red-500 ring-1 ring-red-500'
                                            : 'bg-neutral-950 border-neutral-800 hover:border-neutral-600'
                                            }`}
                                    >
                                        <div className="font-bold relative z-10">{suspect.name}</div>
                                        ...
                                        {isPartnerVote && (
                                            <div className="absolute bottom-0 left-0 right-0 bg-blue-900/50 text-blue-200 text-[10px] py-1 px-2 text-center font-bold">
                                                ORTAĞININ SEÇİMİ
                                            </div>
                                        )}
                                    </button>
                        ...
                        {gameState === 'WON' ? (
                            <>
                                ...
                                <div className="p-6 bg-neutral-900 rounded-2xl border border-green-900/30">
                                    <p className="text-lg">Katil <span className="font-bold text-green-400">Yeğen</span> idi.</p>
                                    <p className="text-neutral-400 mt-2">Kumar borçlarını ödemek için amcasını zehirledi.</p>
                                </div>
                            </>
                        ) : (
                            <>
                                ...
                                <div className="p-6 bg-neutral-900 rounded-2xl border border-red-900/30">
                                    <p className="text-lg">Gerçek katil <span className="font-bold text-red-500">Yeğen</span> idi.</p>
                                </div>
                            </>
                        )}
